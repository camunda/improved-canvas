...
* `FEAT`: source colors from `@bpmn-io/theme`. The `--gray-*` and `--blue-*` variables are removed. Override the `--bio-*` tokens instead.
* `FEAT`: make colors and corner radii themeable
...
* `FEAT`: hide create pad and append idicator while canvas is locked ([#86](https://github.com/camunda/improved-canvas/pull/86))
