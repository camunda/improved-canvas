...
export default class CreatePad {
  ...
  _refresh(target) {
    ...
    // a removed target keeps its businessObject, so canOpen may still pass;
    // getGraphics is falsy once it is off the canvas, so close instead of
    // repositioning against a gone element (which would deref a missing gfx)
    if (!this._canvas.getGraphics(target) || !this.canOpen(target)) {
      this.close();
