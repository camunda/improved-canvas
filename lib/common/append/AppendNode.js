...
AppendNode.prototype.isAllowed = function(target) {
  return !isConnection(target) && this._provider.isAllowed(target);
};
...
AppendNode.prototype._updateAndShow = function(target) {
  const action = this._provider.getActions(),
        additionalEntries = this._provider.getAdditionalEntries ? this._provider.getAdditionalEntries(target) : {},
        node = this.getNode(target, additionalEntries);
...
AppendNode.prototype.getHtml = function(hasAdditionalEntries) {
  return domify(`
    <div class="djs-append-node has-additional-entries">
 ...
 * @return {Overlay}
 */
AppendNode.prototype.getNode = function(target, additionalEntries) {
  if (this.isShown()) {
  ...
  // create node
  const html = this.getHtml(Object.keys(additionalEntries).length > 0);

  Object.entries(additionalEntries).forEach(([id, entry]) => {
    const entryNode = domify(entry.html);

    entryNode.addEventListener('click', function(event) {
      entry.action(event, target);
    });

    html.appendChild(entryNode);
  });
