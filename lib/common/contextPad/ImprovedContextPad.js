...
ImprovedContextPad.prototype._renderTooltips = function({ current }) {
  ...
  entries.forEach((entry)=> {
    ...
    if (entry.classList.contains('call-to-action') && entries[0] === entry) {
      const pad = this._contextPad.getPad(current.element).html;
      if (!pad.classList.contains('call-to-action')) {
        pad.classList.add('call-to-action');
      }
    }
