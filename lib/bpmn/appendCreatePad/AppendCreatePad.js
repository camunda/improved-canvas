...
import { hasCompensateEventDefinition } from 'bpmn-js/lib/util/DiUtil';
import {
  getBusinessObject,
  is
...
export default class AppendCreatePad extends CreatePad {
  ...
  canAppend(target) {
    if (
      ...
      hasCompensateEventDefinition(target) &&
      target.outgoing.some(connection => {
        return is(connection, 'bpmn:Association') &&
          getBusinessObject(connection.target).isForCompensation;
      })
    ) {
      return false;
    }
