...
export default function BPMNImprovedContextPadProvider(contextPad, eventBus, popupMenu, rules) {
...
BPMNImprovedContextPadProvider.prototype.getContextPadEntries = function(target) {
  return (entries) => {
    ...
    // connect moves to the append pad for appendable elements;
    // keep it in context pad for connect-only elements (e.g. participants, end events) that have no append pad
...
BPMNImprovedContextPadProvider.$inject = [ 'contextPad', 'eventBus', 'popupMenu', 'rules' ];